...
async function generateCoverLetter() {
    ...
    // Collect resume data in a structured format for the AI
    const p = resumeData.personal;
    const fullName = `${p.firstName} ${p.lastName}`.trim() || 'Candidate';
    const contact = [p.email, p.phone, p.location].filter(Boolean).join(' | ');
    
    // Format experience
    const experienceText = resumeData.experience.map(e => {
        const dates = e.startDate ? (e.current ? `${e.startDate} - Present` : (e.endDate ? `${e.startDate} - ${e.endDate}` : '')) : '';
        return `${e.position} at ${e.company} ${dates}\n${e.description || ''}`;
    }).join('\n\n');

    // Format education
    const educationText = resumeData.education.map(e => {
        const dates = e.startDate ? (e.endDate ? `${e.startDate} - ${e.endDate}` : e.startDate) : '';
        return `${e.degree} in ${e.field} from ${e.school} ${dates} GPA: ${e.gpa || 'N/A'}`;
    }).join('\n');
...
${skillsText || 'No skills listed'}
    ...
    const systemPrompt = `You are an expert cover letter writer. Using the candidate's resume and the job description, write a professional, tailored cover letter (3-4 paragraphs). Address it to the hiring manager. Include specific skills and experiences from the resume that match the job description. Be concise, enthusiastic, and professional. Do not include placeholders like [Your Name] – use the actual candidate name from the resume. Return only the cover letter text, no additional commentary.`;

    const userMessage = `Resume:\n${resumeContext}\n\nJob Description:\n${jobDesc}`;

    try {
        const letter = await callOpenRouter(systemPrompt, userMessage);
        const cleanedLetter = letter.replace(/\\([()])/g, '$1');
        // Display the result
        const resultDiv = document.getElementById('coverLetterResult');
        const textDiv = document.getElementById('coverLetterText');
        textDiv.textContent = letter;
        // Inside generateCoverLetter() in ai-suggestions.js, after getting letter:
        resumeData.coverLetter.content = letter;
        resumeData.coverLetter.generated = true;
        if (typeof updatePreview === 'function') updatePreview();
        showToast('Cover letter generated!', 'success');
        // Also hide the result div or keep it – we'll add an edit button.
        resultDiv.style.display = 'block';
        
        showToast('Cover letter generated!', 'success');
    } catch (err) {
        showToast('Failed to generate cover letter: ' + err.message, 'danger');
    }
}
